...
export default function useImageDetect() {
  ...
  const detectImage = useCallback(
    async (imageUrl: string): Promise<DetectionResult> => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (res.status === 504) {
            throw new Error(
              "Detection timed out (504). The image might be too large or the server is busy.",
            );
          }
        ...
        const normalizeShape = (
          ...
          const path =
            typeof shape.path === "string" ? shape.path.trim() : null;
          if (!path) return null;
        ...
        const transformed: DetectionResult = {
          ...
          predictions,
          shapes,
          dimension_candidates: Array.isArray(data?.dimension_candidates)
            ? data.dimension_candidates
            : undefined,
