...
export async function POST(request: Request) {
  try {
            ...
            allExtractedText: {
              ...
              items: {
                ...
                properties: {
                  category: {
                    type: Type.STRING,
                    enum: ["room", "dimension", "label", "scale"],
                  },
    ...
    return NextResponse.json(JSON.parse(text) as AIAnalysisResult);
  } catch (error) {
    console.error("Gemini deep scan error:", error);
    return NextResponse.json(
      { error: (error as Error).message || "Deep scan failed." },
      { status: 500 },
    );
  }
}
