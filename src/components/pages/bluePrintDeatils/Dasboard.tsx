...
const Dasboard = ({ blueprintDetails }: any) => {
  ...
  return (
    <section
      className="lg:col-span-2  border border-white/70 relative h-screen
    ...
    >
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 20% 20%, rgba(14,165,233,0.06), transparent 55%), radial-gradient(circle at 80% 0%, rgba(248,113,113,0.06), transparent 45%), radial-gradient(circle at 50% 100%, rgba(59,130,246,0.03), transparent 60%)",
        }}
      />
      ...
      <div className="relative z-10 space-y-10 p-8 lg:p-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            value={blueprintDetails?.images_with_overlay_count}
            delta="↑ 6 Increased from last month"
            gradient
            accentColorClass="from-indigo-400 to-indigo-600"
          />
          <StatCard
            title="Pending Overlays"
            value={
              (blueprintDetails?.total_images ?? 0) -
              (blueprintDetails?.images_with_overlay_count ?? 0)
            }
            delta="↓ 2 Pending"
            gradient
            accentColorClass="from-amber-400 to-amber-500"
          />
          <StatCard
            title="Total Classes"
            value={(blueprintDetails?.class_totals ?? []).length ?? 0}
            delta="+ 4 New"
            gradient
            accentColorClass="from-purple-400 to-purple-600"
          />
        </div>
