...
const QuickStatsGrid: React.FC<QuickStatsGridProps> = ({ stats }) => (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {stats.map(({ label, value, trend, progress, accent, Icon }) => (
            <article
                key={label}
                className="rounded-[28px] border border-white/70 bg-white/75 p-4 shado-[0_20px_45px_rgba(15,23,42,0.12)] backdrop-blur"
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className="text-[0.65rem] font-medium uppercase  text-slate-500">{label}</p>
